...
void enc28j60_ini(void)
{
	...
	enc28j60_writeRegByte(MABBIPG, 0x12); //interval between frames
	enc28j60_writeReg(MAMXFL, MAX_FRAMELEN); //frame max size
	...
	enc28j60_writePhy(PHLCON, PHLCON_LACFG2 | //disable LEDs
...
static uint8_t SPI_ReceiveByte(void)
{
	...
	return bt; //return data register
}
